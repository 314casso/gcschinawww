...
@css[]
<link href="/cms/templates/gcs/css/admin.css" rel="stylesheet" type="text/css" />
...
^if($field ne "uri" && $field ne "key_value" && $field ne "table_name" && $field ne "key_name" && $field ne "upload_dir"){
...
^form:fields.foreach[field;value]{
	^if($form:$field is "file"){
		...
		^form:$field.save[binary;/files/^if(def $form:upload_dir){$form:upload_dir}{unsorted}/${key_value}.^file:justext[$form:$field.name]]
################################ создаем превьюшки
#		^use[photos.p]
#		^photos:build_preview[/files/photos_original^if($form:section_id ne "0"){_}/^if($form:section_id ne "0"){$form:section_id}{$form:document_id}/${just_inserted_file}.^file:justext[$form:$field.name];/files/photos_watermarked^if($form:section_id ne "0"){_}/^if($form:section_id ne "0"){$form:section_id}{$form:document_id}/${just_inserted_file}.^file:justext[$form:$field.name];$MAIN:sites.preview_watermarked;/files/watermarks/${MAIN:sites.id}.png]
#		^photos:build_preview[/files/photos_original^if($form:section_id ne "0"){_}/^if($form:section_id ne "0"){$form:section_id}{$form:document_id}/${just_inserted_file}.^file:justext[$form:$field.name];/files/photos_preview1^if($form:section_id ne "0"){_}/^if($form:section_id ne "0"){$form:section_id}{$form:document_id}/${just_inserted_file}.^file:justext[$form:$field.name];$MAIN:sites.preview_1]
#		^photos:build_preview[/files/photos_original^if($form:section_id ne "0"){_}/^if($form:section_id ne "0"){$form:section_id}{$form:document_id}/${just_inserted_file}.^file:justext[$form:$field.name];/files/photos_preview2^if($form:section_id ne "0"){_}/^if($form:section_id ne "0"){$form:section_id}{$form:document_id}/${just_inserted_file}.^file:justext[$form:$field.name];$MAIN:sites.preview_2]
	}
}
...
^void:sql{update $form:table_name set ^fields_.menu{ $form_name[$fields_.form_name]$field_name[$fields_.field_name]$field_name = '$form:$form_name' }[, ] where $form:key_name = '$key_value'}
...
^switch[$form:todo]{
	...
	^case[DEFAULT]{
		...
		$MAIN:menu[^table::sql{select id, menu_title, menu_uri, visible from ${MAIN:sql_config.table_prefix}menu where menu_id = '^form:id.int(0)' and menu_parent_id = '0' order by order_by ASC}]
		^if($MAIN:menu){
			<ul class="sortable">
			^MAIN:menu.menu{
