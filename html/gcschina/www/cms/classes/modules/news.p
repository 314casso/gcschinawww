...
^if(^uri:path_last.pos[.html] > 0){
	...
	^if($f){
		...
		^if($f){
			<a href="${MAIN:language_prefix}/news/">Смотрите также:</a>
			<ul>
				^f.menu{
					<li /><a href="${MAIN:language_prefix}/news/${f.id}.html">$f.title</a> ^f.published.left(10)
				}
			</ul>
		}
	...
	<div id="faq">
		...
		^f.menu{
			<li>
				<a href="${MAIN:language_prefix}/news/${f.id}.html">$f.title</a>^if(def $f.announce){^untaint{$f.announce}}<p />^f.published.left(10)
			</li>
		}
		</ul>
	</div>
